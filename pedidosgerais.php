...
if(isset($_POST['aceitar'])){
	$idpraatt = $_POST['idpraatt'];
	require "conexao.php";
	$sqlatt = "UPDATE pedido SET situacao = 'Pedido aceito' WHERE id_pedido = '$idpraatt'";
	mysqli_query($conn, $sqlatt) or die("Não foi possível ".mysqli_error());
	echo "<script>alert('Pedido aceito com sucesso!');</script>";
	echo "<meta http-equiv='refresh' content='0.1;pedidosgerais.php'>";
}
if(isset($_POST['iddopedido'])){
	$idcliente = $_POST['iddopedido'];
	$user = $_POST['usuario'];
}
else{
...
<body class="degrade">
	<div class="container" style="background: white;">
		<center>
			<h3 style="color: red; margin-bottom: 30px; margin-top: 30px;"><?php echo "Pedidos de $user"; ?></h3><br>
		...
		{
			$sqlpedido = "SELECT pedido.id_pedido, pedido.endereco, pedido.complemento, pedido.bairro, pedido.cidade, pedido.data, pedido.uf, pedido.situacao, pedido.nome, produtos.foto, produtos.tipo, pedido.quantidade AS quantidadet, pedido.total, produtos.id_produto FROM pedido INNER JOIN produtos ON id_cliente = '$idcliente' AND pedido.id_produto = produtos.id_produto;";
			...
			while($row2 = mysqli_fetch_assoc($result2))
			{
				...
				<div class="row">
					...
					<div class="col-sm">
						...
						<b>Total a pagar: </b>R$ <span style="color: red;"><?php echo $row2['total']; ?></span><br>
						...
						<form action="pedidosgerais.php" method="POST">
							...
							<form action="pedidosgerais.php" method="POST">
								<input type="hidden" name="idpraatt" value="<?php echo $row2['id_pedido']; ?>">
								<input type="submit" name="aceitar" style="margin: auto;" class="btn btn-info btn-sm" value="Aceitar pedido">
							</div>
						</form>
					</div>
				</div><br><br>
